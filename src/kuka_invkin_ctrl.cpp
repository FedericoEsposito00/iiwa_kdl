...
void KUKA_INVKIN::ctrl_loop() {
	...
	float i_cmd[7];
	i_cmd[0] = 0.0;
	i_cmd[1] = i_cmd[2] = i_cmd[4] = i_cmd[6] = 0;
	i_cmd[3] = -1.57;
	i_cmd[5] = 1.57;
	...
	//cout<<_p_out.p.x()<<endl<<_p_out.p.y()<<endl<<_p_out.p.z()<<endl;
	...
	while(ros::ok()) {
		...
		F_dest.p.data[0] = P_x[j];
		F_dest.p.data[1] = P_y[j];
		...
		if (j == N) {
			j = 0;
			cout<<"Restarting the trajectory!\n";
		}
